...
use skia as skia_safe;
use skia_safe::{gradient_shader, Color, Matrix, Paint, PaintJoin, PaintStyle, Path, Point, TileMode};
use std::cmp::min;
...
fn point_in_circle(center: (f32, f32), radius: f32, radians: f32) -> (f32, f32) {
	(center.0 + radius * radians.cos(), center.1 - radius * radians.sin())
}

pub fn render_frame(frame: usize, fps: usize, bpm: usize, canvas: &mut skia_safe::canvas::Canvas) -> usize {
...
fn chain_ring(canvas: &mut skia_safe::canvas::Canvas,
              center: (i32, i32),
              radius: i32,
              rotation: f32,
              teeth_count: i32)
...
fn triangle(canvas: &mut skia_safe::canvas::Canvas,
            center: (i32, i32),
            radius: i32,
            degrees: f32,
            vertex: Option<i32>,
